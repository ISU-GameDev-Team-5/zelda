...
@onready var buy_button: Button = $MarginContainer/ColorRect/HBoxContainer/MerchantColumn/BuyButton
@onready var sell_button: Button = $MarginContainer/ColorRect/HBoxContainer/PlayerColumn/SellButton
...
func setup_selling_grid():
	...
	for i in items_to_sell.size():
		...
		selling_slot.slot_clicked.connect(on_sell_slot_clicked.bind(i))

func on_sell_slot_clicked(idx: int):
...
func _on_sell_button_pressed() -> void:
	...
	for i in selected_sell_item_indexes:
		...
		var current_items_in_player_inventory = (player.find_child("Inventory") as Inventory).items
		...
		inventory.add_item(GOLD_COIN, gold_to_add_to_player)
